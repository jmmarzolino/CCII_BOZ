...
for (m in gens) {
  OutName = paste(paste(m,"He",sep="_"),"dist.png",sep="_")
  OutName2 = paste(paste(m,"weighted_He",sep="_"),"dist.png",sep="_")

  xlab <- paste(m, "Expected Heterozygosity", sep=" ")
  xlab2 <- paste(m, "Weighted Expected Heterozygosity", sep=" ")

  #plotting He
  ggplot(all_He, aes(get(m))) + geom_histogram(bins=15)+ theme_minimal()+xlab(xlab)+ylim(NA,2750000)+xlim(NA,0.5)
  ggsave(OutName, width = 10, height = 8, units = "in")

  #plotting weighted He
  ggplot(all_weighted_He, aes(get(m))) + geom_histogram(bins=15)+ theme_minimal()+xlab(xlab2)+ylim(NA,3500000)+xlim(NA,0.5)
