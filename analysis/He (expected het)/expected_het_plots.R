...
all <- read_delim("all.expected_het", "\t",
+     escape_double = FALSE, col_names = FALSE,
+     trim_ws = TRUE)

generation <- c("F1", "F50", "F27", "F18", "F28", "F58")
binlist <- c(5,10,15, 20, 30, 40, 50)

# when trying to use the double loop, ggplot doesn't want to take/understand the column name being passed from a list so manually changing the aes(X#) and j=# is just faster than spending more time figuring it out
...
for (m in binlist) {
  outname = paste(paste(paste(generation[j], "_Het_expect",sep=""),m,sep="_"),"bins.png",sep="")
  ggplot(all, aes(X5)) + geom_histogram(bins=m)+ theme_minimal()+xlab(xlab)
  ggsave(outname, width = 10, height = 8, units = "in")
}
...
for (m in binlist) {
  outname = paste(paste(paste(generation[j], "_Het_expect",sep=""),m,sep="_"),"bins.png",sep="")
  ggplot(all, aes(X6)) + geom_histogram(bins=m)+ theme_minimal()+xlab(xlab)
  ggsave(outname, width = 10, height = 8, units = "in")
}
...
for (m in binlist) {
  outname = paste(paste(paste(generation[j], "_Het_expect",sep=""),m,sep="_"),"bins.png",sep="")
  ggplot(all, aes(X7)) + geom_histogram(bins=m)+ theme_minimal()+xlab(xlab)
  ggsave(outname, width = 10, height = 8, units = "in")
}
...
for (m in binlist) {
  outname = paste(paste(paste(generation[j], "_Het_expect",sep=""),m,sep="_"),"bins.png",sep="")
  ggplot(all, aes(X8)) + geom_histogram(bins=m)+ theme_minimal()+xlab(xlab)
  ggsave(outname, width = 10, height = 8, units = "in")
}
...
for (m in binlist) {
  outname = paste(paste(paste(generation[j], "_Het_expect",sep=""),m,sep="_"),"bins.png",sep="")
  ggplot(all, aes(X9)) + geom_histogram(bins=m)+ theme_minimal()+xlab(xlab)
  ggsave(outname, width = 10, height = 8, units = "in")
}
...
for (m in binlist) {
  outname = paste(paste(paste(generation[j], "_Het_expect",sep=""),m,sep="_"),"bins.png",sep="")
  ggplot(all, aes(X10)) + geom_histogram(bins=m)+ theme_minimal()+xlab(xlab)
  ggsave(outname, width = 10, height = 8, units = "in")
}
